...
class Maze:
    ...
    def genRandomMazeWalls(self):
        self.hwalls = [ map(lambda _: random.choice((0,1)), [0] * (self.w-1)) for _ in range(self.h) ]
        self.vwalls = [ map(lambda _: random.choice((0,1)), [0] * (self.w)) for _ in range(self.h-1) ]
